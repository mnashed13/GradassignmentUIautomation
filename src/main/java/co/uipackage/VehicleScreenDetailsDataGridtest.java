...
public class VehicleScreenDetailsDataGridtest {
	WebDriver driver;

	public VehicleScreenDetailsDataGridtest(WebDriver driver) {
		this.driver=driver;
}
...
// dropdown	
	...
	public void DetailFormInputsDataGrid(String year, String Make,String Shape, String Usage, String PostCode) throws InterruptedException {
		Thread.sleep(5000);
		System.out.println("input detail teststarted ");
		YearOfManufactureButton.sendKeys(year);
		Thread.sleep(5000);
...
//		MakeOfVehicleBtn.getOptions().get(1).click();
		...
		Select  VehicleShapeBtn = new Select(VehicleShape);
		 VehicleShapeBtn.selectByVisibleText(Shape);
...
//		VehicleUsageBtn.getOptions().get(1).click();
		...
		postcodeBtn.selectByVisibleText(PostCode);
